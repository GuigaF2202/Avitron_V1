...
const ResendVerification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(response.data.message || 'Email de verificação reenviado com sucesso! Por favor, verifique sua caixa de entrada.');
      setEmail('');
    } catch (error) {
      setError(error.response?.data?.message || 'Não foi possível reenviar o email de verificação. Tente novamente mais tarde.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white py-8">
      <div className="w-full max-w-md px-6">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400">Insira seu email para receber um novo link de verificação</p>
        </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-2xl shadow-xl overflow-hidden">
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="mt-6 text-center text-sm text-gray-400">
              <Link to="/auth" className="text-indigo-400 hover:text-indigo-300 transition-colors">Voltar para Login</Link>
            </div>
          </form>
        </div>
      </div>
